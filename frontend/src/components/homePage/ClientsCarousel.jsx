...
const ClientsCarousel = () => {
  ...
  return (
    <section className="w-full bg-white dark:bg-[#0B1120] md:py-20 overflow-hidden">
      <style>{marqueeStyles}</style>

      {/* ── Section heading ── */}
      <div className="text-center mb-14 px-6">
        <h2 className="font-jetbrains text-[27px] md:text-[64px] font-normal leading-[1.2] tracking-normal text-black dark:text-white">
          Our Trusted Clients
        </h2>
      </div>
      ...
      >
        <div className="marquee-track">
          {doubled.map((logo, i) => (
            <div
              key={i}
              className="flex items-center justify-center mx-5 md:mx-10 shrink-0 w-20 h-20 md:w-32 md:h-32 bg-white dark:bg-gray-100 rounded-2xl shadow-sm border border-gray-100 dark:border-none p-3 md:p-5 transition-all duration-300"
            >
              <img
                src={logo.src}
                alt={logo.alt}
