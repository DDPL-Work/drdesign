...
import { gisCardsData } from "../../constants/servicesData";
...
const GISPractice = () => {
  ...
  useEffect(() => {
    if (location.hash === "#gis-practice" && location.state?.scrollToCard !== undefined) {
      const idx = location.state.scrollToCard;
      setTimeout(() => {
        if (isMobile) {
          const mobileCard = document.getElementById(`gis-card-mobile-${idx}`);
          if (mobileCard) {
          ...
          const desktopCard = document.getElementById(`gis-card-desktop-${idx}`);
          ...
          <div className="w-full overflow-visible relative">
            ...
            >
              {gisCardsData.map((card, index) => (
                <div key={card.id} id={`gis-card-desktop-${index}`} className="w-[400px] lg:w-[476px] shrink-0 flex">
                  <PracticeCard card={card} />
                </div>
              ))}
            </div>
